...
{
    class NPC_ShadowLurker : IEnemyObjects 
    {
        // This represents 3D models in memory.
        public Model model { get; protected set; }

        // Get state list
        public enum lurkerBehavior { Idle, Passive, Alarm, Hostile, Fleeing }

        // This world for this particular model.
        protected Matrix world = Matrix.Identity;


        public NPC_ShadowLurker(Model m) // Constructor
        {
            model = m;
        }
        ...
        public void setBehavior(lurkerBehavior b)
        {
            switch (b)
            {
                case lurkerBehavior.Idle:
                    // have no state running in updater
                    break;
                case lurkerBehavior.Passive:
                    // have "wandering" state running in updater
                    break;
                case lurkerBehavior.Alarm:
                    // have "alert others" state running in updater
                    break;
                case lurkerBehavior.Hostile:
                    // have beast target random player and attack
                    break;
                case lurkerBehavior.Fleeing:
